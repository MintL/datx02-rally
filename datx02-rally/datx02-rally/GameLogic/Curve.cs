...
namespace datx02_rally.GameLogic
{
    ...
    class Curve
    {
        ...
        public Curve(float side)
        {
            side *= 1.5f / 3f;
            float variation = 1f;

            nodes.Add(new CurveNode() {
                Position = new Vector3(0, 0, -side),
                Tangent = Vector3.Transform(new Vector3(variation * side, 0, 0), Matrix.CreateRotationY(MathHelper.PiOver2 * (float)(2 * random.NextDouble() - 1)))
            });
            nodes.Add(new CurveNode()
            {
                Position = new Vector3(side, 0, 0),
                Tangent = Vector3.Transform(new Vector3(0, 0, variation * side), Matrix.CreateRotationY(MathHelper.PiOver2 * (float)(2 * random.NextDouble() - 1)))
            });
            nodes.Add(new CurveNode()
            {
                Position = new Vector3(0, 0, side),
