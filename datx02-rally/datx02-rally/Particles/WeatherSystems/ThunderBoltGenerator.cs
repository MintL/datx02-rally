...
namespace datx02_rally.Particles.WeatherSystems
{
    class ThunderBoltGenerator : GameComponent
    {
        ...
        {
            if (random.NextDouble() < .002 || Game.GetService<InputComponent>().GetKey(Microsoft.Xna.Framework.Input.Keys.B))
            {
                flash = 1;
                ...
                branches.Add(new SubBolt()
                {
                    ...
                    target = endPosition + new Vector3(
                        20000 * ((float)random.NextDouble() - .5f), 0,
                        20000 * ((float)random.NextDouble() - .5f))
                });
